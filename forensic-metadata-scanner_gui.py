...
class PDF(FPDF):
    def footer(self):
        ...
        self.cell(
            0, 10,
            f'Página {self.page_no()} - Generado el {datetime.datetime.now().strftime("%d/%m/%Y %H:%M")}',
            align='C'
        )
...
class AnalizadorForenseGUI:
    def __init__(self, root):
        ...
        self.crear_interfaz()
        self.localizar_exiftool()

    # === ExifTool desde PATH ===
    def localizar_exiftool(self):
        self.ruta_exiftool = shutil.which("exiftool")
        if not self.ruta_exiftool:
            messagebox.showerror(
                "ExifTool no encontrado",
                "ExifTool no está instalado o no está en el PATH del sistema.\n\n"
    ...
    def iniciar_analisis(self):
        ...
        self.proceso_hilo = threading.Thread(
            target=self.generar_informe_forense,
            args=(carpeta,),
            daemon=True
        )
    ...
    def generar_informe_forense(self, ruta_carpeta):
        try:
            ...
            self.escribir("Herramienta utilizada: ExifTool", "seccion")
            self.escribir("")
